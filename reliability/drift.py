...
---------------------
Enterprise distribution drift detection.

Methods: PSI (Population Stability Index), KL Divergence, feature-level drift.
"""
...
def _psi_single(ref: np.ndarray, prod: np.ndarray, n_bins: int = 10) -> float:
...
def _kl_divergence(ref_probs: np.ndarray, prod_probs: np.ndarray, n_bins: int = 20) -> float:
...
def _interpret_psi(psi: float) -> str:
    if psi < 0.10:  return "No significant shift"
    if psi < 0.20:  return "Moderate shift — monitor closely"
    return "Significant shift — investigate immediately"
...
def compute_prediction_drift(
    ...
    """Compare model output distributions between reference and production."""
...
def compute_feature_drift(
    ...
    """Compute per-feature PSI and aggregate drift score."""
    ...
    if not common_cols:
        return {"features": {}, "overall_drift_score": 0.0, "n_features": 0,
                "overall_interpretation": "No common numeric features found."}
    ...
    for col in common_cols:
        ...
        prod_col = prod_df[col].dropna().values
        ...
        feature_stats[col] = {
            "psi":                      psi,
            "mean_ref":                 round(float(ref_col.mean()),  4),
            "mean_prod":                round(float(prod_col.mean()), 4),
            "mean_shift":               round(mean_shift, 4),
            "std_ref":                  round(float(ref_col.std()),   4),
            "std_prod":                 round(float(prod_col.std()),  4),
    ...
    return {
        ...
        "overall_interpretation": _interpret_psi(overall),
...
def drift_heatmap(
    ...
    """Bar chart: PSI per feature with colour coding and threshold lines."""
    ...
    colors    = ["#2ecc71" if p < 0.10 else "#f39c12" if p < 0.20 else "#e74c3c" for p in psi_vals]

    fig, ax = plt.subplots(figsize=(max(7, len(names) * 0.9), 5))
    ax.set_facecolor("#f9f9f9")
    bars = ax.bar(names, psi_vals, color=colors, edgecolor="white", alpha=0.9)

    ax.axhline(y=0.10, color="#f39c12", linestyle="--", linewidth=1.2, label="Moderate (0.10)")
    ax.axhline(y=0.20, color="#e74c3c", linestyle="--", linewidth=1.2, label="Significant (0.20)")
