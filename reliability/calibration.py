...
---------------------------
Enterprise calibration analysis: ECE, MCE, reliability diagrams, confidence histograms.

All chart functions return bytes (PNG) when return_bytes=True for embedding in PDF/UI.
Dark theme optimised for the AegisML Streamlit dashboard.
"""
...
def compute_ece(y_true: np.ndarray, y_prob: np.ndarray, n_bins: int = 10) -> dict:
    """
    Compute Expected Calibration Error (ECE) and related calibration metrics.

    Parameters
    ----------
    y_true  : array-like of int (0/1)
    y_prob  : array-like of float (0–1)
    n_bins  : number of equal-width bins

    Returns
    -------
    dict with ece, mce, overconfidence_gap, n_bins, bins (per-bin details)
    """
    ...
    for i in range(n_bins):
        ...
        if bs == 0:
            bins.append({
                ...
                "count": 0, "accuracy": None, "confidence": None, "calibration_error": None,
            })
            continue

        ba = float(y_true[mask].mean())
        bc = float(y_prob[mask].mean())
        ce = abs(ba - bc)
        ece += (bs / n) * ce
        mce = max(mce, ce)
        total_oc += (bs / n) * (bc - ba)
    ...
    return {
        "ece": round(ece, 4),
        "mce": round(mce, 4),
        "overconfidence_gap": round(total_oc, 4),
        "n_bins": n_bins,
        "bins": bins,
    }
...
def reliability_diagram(
    ...
    """Reliability diagram: predicted probability vs fraction of positives per bin."""
    ...
    axb = fig.add_subplot(gs[3, 0], sharex=ax)
    _dark(fig, [ax, axb])
    ...
    handles = [
        mpatches.Patch(color=ORANGE, alpha=0.8, label="Overconfident"),
    ...
    if return_bytes:
        buf = BytesIO()
        fig.savefig(buf, format="png", dpi=150, bbox_inches="tight", facecolor=BG_VOID)
        plt.close(fig)
        buf.seek(0)
        return buf.read()
    plt.show()
...
def confidence_histogram(
    ...
    """Confidence score histogram with KDE overlay and summary statistics."""
    y_prob = np.asarray(y_prob)

    fig, axes = plt.subplots(
        1, 2, figsize=(12, 5), facecolor=BG_VOID,
    ...
    try:
        ...
        ys  = kde(xs) * len(y_prob) * (1.0 / n_bins_h)
    ...
    stats = [
        ("SAMPLES", f"{len(y_prob):,}"),
        ("MEAN",    f"{mp:.4f}"),
        ("MEDIAN",  f"{float(np.median(y_prob)):.4f}"),
        ("STD",     f"{float(y_prob.std()):.4f}"),
        ("MIN",     f"{float(y_prob.min()):.4f}"),
        ("MAX",     f"{float(y_prob.max()):.4f}"),
        ("HIGH CONF", f"{(y_prob >= 0.7).mean():.1%}"),
        ("LOW CONF",  f"{(y_prob <= 0.3).mean():.1%}"),
    ]
...
def calibration_error_heatmap(
    y_true: np.ndarray,
    y_prob: np.ndarray,
    n_bins: int = 10,
    return_bytes: bool = False,
):
    """Diverging bar chart: per-bin calibration gap (overconfidence/underconfidence)."""
    cal   = compute_ece(y_true, y_prob, n_bins=n_bins)
    bins  = [b for b in cal["bins"] if b["count"] > 0]
    centers = [(b["bin_lower"] + b["bin_upper"]) / 2 for b in bins]
    accs    = [b["accuracy"]  for b in bins]
    ...
    fig, (ax1, ax2) = plt.subplots(
        2, 1, figsize=(10, 7), facecolor=BG_VOID,
    ...
    ax1.set_title(" Calibration Error Analysis — Per-Bin Gap", fontsize=12, fontweight="bold", loc="left", pad=12)
    ax1.tick_params(labelbottom=False)
    ax1.legend(
        handles=[mpatches.Patch(color=ORANGE, alpha=0.85, label="Overconfident"),
    ...
    for cx, n_val, e in zip(centers, nv, errs):
        col = plt.cm.YlOrRd(0.2 + n_val * 0.8)
